...
async def on_pago(ctx: Context):
	...
	credito = credit[0]
	gun.close()
	if credito >= int(debito) and len(debito) >= 1 and len(userto) >= 1:
	  ...
	  await ctx.send("No tienes Eskoins suficientes. O cometiste un error al escribir el comando.")
...
async def on_apuesta(ctx: Context):
  ...
  mlb = sa.fetchone()
  gta = mlb[0]
  if gta >= spn and len(spn) >= 1 and len(cant) >= 1:
    f = [1, 2, 3, 4, 5, 6]
    trg = random.choice(f)
    if trg == cant:
      suma = int(spn) * trg
